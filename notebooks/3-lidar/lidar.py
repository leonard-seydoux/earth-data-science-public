...
def plot_ternary(x, y, ax=None, density=True, **kwargs):
    ...
    **kwargs : dict, optional
    ...
    if density and "c" not in kwargs:
        try:
            density = gaussian_kde(points)(points)
        except:
            density = np.ones_like(x)
        kwargs.setdefault("c", density)
    ...
    if density is not False and "c" not in kwargs:
        # Colorbar
        plt.colorbar(
            mappable,
...
def calculate_eigenvalues(
    ...
    # Calculate the covariance in parallel with tqdm
    with Pool(processes=64) as pool:
        eigenvalues = pool.map(eigs_partial, selected_points)
        # eigenvalues = list(
        #     tqdm(
        #         pool.imap(eigs_partial, selected_points),
